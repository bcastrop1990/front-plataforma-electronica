...
@Component({
  selector: 'app-step1-datos-solicitante',
  templateUrl: './step1-datos-solicitante.component.html',
  styleUrls: ['./step1-datos-solicitante.component.scss']
})
export class Step1DatosSolicitanteComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.form = this.formBuilder.group({
      ...
      celular: ['', [Validators.required, Validators.minLength(6), Validators.maxLength(12), Validators.pattern('^[0-9]*$')]],
      celularConfirm: ['', [Validators.required, Validators.minLength(6), Validators.maxLength(12), Validators.pattern('^[0-9]*$')]],
    });
  }
  ...
  btnNext(stepper: MatStepper) {
    ...
    if (celular !== celularConfirm) {
      this.utilService.getAlert('Aviso', 'El número de teléfono / celular no coincide.');
      return;
    }
